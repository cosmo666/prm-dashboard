import { DatePreset } from 'src/app/core/store/filter.store';
import { POC_TODAY } from './poc-today';

// Re-export so existing call sites can import POC_TODAY from this module too.
export { POC_TODAY } from './poc-today';
...
export function resolvePreset(preset: DatePreset, today: Date = POC_TODAY): PresetRange {
  ...
  const def = PRESET_DEFS.filter(p => p.key === preset)[0];
  ...
  switch (preset) {
    ...
    case 'yesterday':     {
      ...
      return { from: iso(d), to: iso(d), label };
    }
    ...
    case 'last_3_months': return { from: iso(new Date(y, m - 2, 1)), to: iso(today), label };
    case 'last_6_months': return { from: iso(new Date(y, m - 5, 1)), to: iso(today), label };
