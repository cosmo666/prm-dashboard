...
export class ServiceBreakupTabComponent implements OnInit, OnDestroy {
  ...
  // on this branch yet).
  trendSeries$ = new BehaviorSubject<LineSeries[]>([]);
  ...
  ngOnInit(): void {
    this.filters.queryParams$.pipe(
      ...
      r => {
        ...
        // Monthly trend — top 5 services by total count, rendered as a
        // stacked area line chart (matches Angular 17 main). Each series
        // is one IATA SSR code; tooltips trigger on the axis line so all
        // 5 values appear together at the hovered month. The line-chart
        // component adds a soft area gradient per series under stacked
        // mode, producing the layered "ribbon" look the design calls for.
        ...
        this.trendSeries$.next(top5.map(t => ({
          name: t,
          data: rows.map(row => [row.month, row.counts[t]] as [string, number]),
          color: SSR_COLORS[t] || '#94a3b8',
        })));
