...
frontend-v8/                                # Angular 8.2 + PrimeNG 8.0.3 — host :4300 (NgModules, BehaviorSubject stores)
  ...
  src/app/                                 # Mirrors Angular 17 feature layout — 5-tab dashboard, same DTOs
    ...
    shared/{charts,components,pipes}/      # ECharts wrappers, form-field, toast, progress-bar, etc.
docs/
...
| 2026-05-11 | Dual frontends on `main`: Angular 17 (`frontend/`, :4200) and Angular 8 + PrimeNG (`frontend-v8/`, :4300) share the same backend | The user's host application is Angular 8 + PrimeNG 8.0.3; the v8 build was imported via `git read-tree --prefix=frontend-v8/` from a transient `angular-8-rewrite` branch so it lives alongside the Angular 17 UI on `main`. Both frontends proxy `/api` to the same Ocelot gateway and read the same per-tenant Parquet files — switching ports is the only difference at runtime. CORS allowlists on auth/tenant/prm/gateway extended to include `{localhost,aeroground.localhost,skyserve.localhost,globalprm.localhost}:4300`. |
| 2026-05-11 | `angular-8-rewrite` branch + `.worktrees/` directory deleted after consolidation | Once both frontends were live on `main` and verified end-to-end, the rewrite branch was deleted locally and on `origin` (the v8 *files* live on at `frontend-v8/` on main; only the commit history of the rewrite is gone). One branch policy: `main` is the only branch going forward. |
| 2026-05-11 | Per-service DuckDB `memory_limit` via `DataPath:MemoryLimit` config (env-overridable as `DataPath__MemoryLimit`) | DuckDB's default is ~80% of host RAM **per connection**; with `PoolSize=16` that's effectively uncapped — 16 engines each thinking they own most of memory. `DuckDbContext` now applies `SET memory_limit='…'` inside the `(state != Open)` branch of `AcquireAsync`, so the PRAGMA runs on first open AND any self-healing re-open without running on every acquire. `DataPathOptions.MemoryLimit` validated at startup against DuckDB's accepted size forms (e.g. `2GB`, `512MiB`, `60%`). Compose defaults: `prm=2GB`, `auth=256MB`, `tenant=256MB` — Auth/Tenant just look up rows in small master tables, only PrmService runs heavy aggregations. Overridable via `PRM_DUCKDB_MEMORY_LIMIT` / `AUTH_DUCKDB_MEMORY_LIMIT` / `TENANT_DUCKDB_MEMORY_LIMIT`. |
| 2026-05-11 | SavedViews + CommandPalette features dropped from both frontends | Originally Phase-6 polish in the v8 rewrite (and ported to v17 first). Both features overlapped with the existing URL-synced filter state — back/forward, copy-link sharing, and explicit date presets already covered what most users needed. Keeping them added UI complexity and an extra `localStorage` keyspace per tenant without clear demand. Removed `frontend/src/app/core/store/saved-views.store.ts`, both `saved-views-menu/` and `command-palette/` folders in v17, the v8 `command-palette/` folder, the `<app-command-palette>` mounts in both `AppComponent`s, the v17 filter-bar `<app-saved-views-menu>` wiring, the v17 portaled menu CSS in `styles.scss`, and the now-dead `requestedTab$` tab-switch channel from v8 `NavigationStore`. Phase 6 in the spec updated to reflect the cut. |
