...
> The snippets below use `WHERE loc_name = :airport` to keep them readable. For multi-airport filtering swap in `WHERE loc_name IN (:airports)` — the API accepts CSV on `?airport` and `BaseQueryService.BuildWhereClause` picks the right branch (single → equality, multi → `IN`). All queries run against per-tenant Parquet via DuckDB; use DuckDB dialect (see the integer-division note below).
...
```sql
-- Per-id active minutes, then average. DuckDB dialect: // is integer division
-- (truncating); plain / on integers returns DOUBLE and CAST rounds rather
-- than truncates — always use // for HHMM → minutes conversion.
-- In C# code this pattern lives in HhmmSql.ActiveMinutesExpr which handles
-- the COALESCE(paused_at, end_time) and GREATEST(..., 0) clamps.
SELECT AVG(total_minutes) FROM (
  SELECT id, SUM(
    (COALESCE(paused_at, end_time) // 100) * 60 + (COALESCE(paused_at, end_time) % 100)
...
- SQL schema (legacy MySQL source for the Parquet pipeline): `database/init/02-tenant-schema.sql`
- Per-tenant runtime data: `data/{tenant-slug}/prm_services.parquet` (generated by `backend/tools/PrmDashboard.ParquetBuilder/`)
- SQL helpers: `backend/src/PrmDashboard.PrmService/Sql/HhmmSql.cs` (`ToMinutes`, `ActiveMinutesExpr`)
- Query base class: `backend/src/PrmDashboard.PrmService/Services/BaseQueryService.cs` (`BuildWhereClause`, `ResolveTenantParquet`, `GroupCountAsync`, `DistinctAsync`, `MinMaxDateAsync`, `GetPrevPeriodStart`)
- Time helpers (C# equivalents, used by tests + tools): `backend/src/PrmDashboard.Shared/Extensions/TimeHelpers.cs`
