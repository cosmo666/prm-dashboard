...
> Project is on the **`angular-8-rewrite`** branch. The Angular 17 / NgRx Signal Store / standalone-component frontend on `main` has been replaced with Angular 8.2.14 + PrimeNG 8.0.3 to satisfy a host-app integration constraint. This rule file describes the Angular 8 reality, not the historical one. Anything you remember from the previous file (signals, NgRx, standalone components, Angular Material 3, ESLint, Vite) is **wrong on this branch**.
